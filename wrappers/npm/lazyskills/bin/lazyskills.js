...
const RELEASE_BASE_URL =
  process.env.LAZYSKILLS_RELEASE_BASE_URL ||
  `https://github.com/Alpha-Innovation-Labs/lazyskills/releases/download/v${VERSION}`;
...
async function ensureManagedBinary(target) {
  ...
  const assetName = `lazyskills-${target.triple}${
    target.binaryName.endsWith(".exe") ? ".exe" : ""
  }`;
  const url = `${RELEASE_BASE_URL}/${assetName}`;

  console.error(`Installing lazyskills ${VERSION} (${target.triple})...`);
...
async function main() {
  ...
  if (!target) {
    console.error(
      `Unsupported platform for lazyskills wrapper: ${process.platform}/${process.arch}`,
    );
    process.exit(1);
  }
  ...
  try {
    ...
    console.error(`Failed to install lazyskills binary: ${error.message}`);
    console.error(
